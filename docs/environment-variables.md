...
channels through TUNA.

If the corresponding tools are already installed and owned by OMD, the source
profile is also persisted through native configuration:
...
OMD only writes absent files or exact files previously generated by OMD.
User-owned or modified files are preserved. Explicit source switching fails and
rolls the transaction back when such a conflict prevents a complete migration.
