...
component="neovim"
bin_path="$(omd_module_bin_dir)/nvim"
install_dir="${OHMYDEVPOD_NEOVIM_DIR:-$(omd_module_prefix)/opt/neovim}"

status() {
  if managed; then
    [[ -x "${install_dir}/bin/nvim" ]] &&
      [[ -L "${bin_path}" ]] &&
      [[ "$(readlink "${bin_path}")" == "${install_dir}/bin/nvim" ]]
  else
    command -v nvim >/dev/null 2>&1 || [[ -x "${bin_path}" ]]
  fi
}
managed() {
...
install_or_update() {
  ...
  if omd_module_path_exists "${install_dir}" && ! managed; then
    omd_module_info error "refusing to overwrite unmanaged Neovim directory: ${install_dir}"
    return 1
  fi
  if omd_module_dry_run "$@"; then
    omd_module_info plan "${action} vendored Neovim under ${install_dir}"
    return 0
  fi
  ...
  OHMYDEVPOD_ASSET_ROOT="${OHMYDEVPOD_ASSET_ROOT:-${repo_root}/vendor/releases}" \
    ...
    OHMYDEVPOD_NEOVIM_DIR="${install_dir}" \
...
uninstall() {
  ...
  if omd_module_dry_run "$@"; then
    omd_module_info plan "remove managed Neovim directory and launcher"
    return 0
  fi
  rm -f "${bin_path}"
