...
component="zellij"
bin_path="$(omd_module_bin_dir)/zellij"

status() {
  if managed; then
    [[ -x "${bin_path}" ]]
  else
    command -v zellij >/dev/null 2>&1 || [[ -x "${bin_path}" ]]
  fi
}
...
install_or_update() {
  ...
  if omd_module_dry_run "$@"; then
    omd_module_info plan "${action} vendored Zellij binary at ${bin_path}"
    return 0
  fi
...
uninstall() {
  ...
  if omd_module_dry_run "$@"; then
    omd_module_info plan "remove managed Zellij binary ${bin_path}"
    return 0
  fi
  rm -f "${bin_path}"
