...
component="witr"
bin_path="$(omd_module_bin_dir)/witr"

status() {
  if managed; then
    [[ -x "${bin_path}" ]]
  else
    command -v witr >/dev/null 2>&1 || [[ -x "${bin_path}" ]]
  fi
}
...
install_or_update() {
  ...
  if omd_module_dry_run "$@"; then
    omd_module_info plan "${action} vendored witr binary at ${bin_path}"
    return 0
  fi
  ...
  OHMYDEVPOD_ASSET_ROOT="${OHMYDEVPOD_ASSET_ROOT:-${repo_root}/vendor/releases}" \
    ...
    bash "${repo_root}/build/install-witr.sh"
...
uninstall() {
  ...
  if omd_module_dry_run "$@"; then
    omd_module_info plan "remove managed witr binary ${bin_path}"
    return 0
  fi
  rm -f "${bin_path}"
